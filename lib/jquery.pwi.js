...
pwi = (function ($) {
    ...
    $.fn.pwi = function (opts) {
        ...
        function photo(j, hidden, username, extDataMap) {
...
// console.log('Appending: ' + j.georss$where.gml$Point.gml$pos.$t);
		...
		extDataMap = {'longitude': longitude, 'latitude': latitude, 'caption': $caption, 'linkdata': linkName };
console.log('WTF: ' + JSON.stringify(extDataMap));
        ...
        function album(j) {
            ...
            for (var i = 0; i < $np; i++)
            {
                var $scPhoto = photo(j.feed.entry[i], !((i >= startShow) && (i < endShow)), $relUsername, extDataMap);
        ...
        function latest(j) {
            ...
            while (i < settings.maxResults && i < $len) {
                var  $scPhoto = photo(j.feed.entry[i], false, $relUsername, extDataMap);
                $scPhotos.append($scPhoto);
                i++;
            }
            $scPhotos.append(strings.clearDiv);
    ...
    return { extDataMap: JSON.stringify(extDataMap) };
})(jQuery);
