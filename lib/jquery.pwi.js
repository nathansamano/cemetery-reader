...
// A little mod here to show filename on page
// This original way was more work than the current one shown below
// var linkName =j.media$group.media$content[0].url.substr(j.media$group.media$content[0].url.lastIndexOf('/') + 1);
var linkName = j.media$group.media$title.$t;
// console.log('Generated: ' + linkName);
                            $c += '<p><a href="' + j.media$group.media$content[0].url + '">' +
                                linkName + '</a>';
                        }
                        $html.append($c);
// Here is where we could extract the Point data
// Also note j.media$group.media$content[0].url contains the filename
//       as the last element
//
// If this image is geotagged, extract the Point data and append it to photo
