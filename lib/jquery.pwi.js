...
pwi = (function ($) {
    ...
    $.fn.pwi = function (opts) {
	    ...
	    // Someday I'll know why this has to be here!
	    returnDataMap = $.fn.pwi.returnDataMap;
        ...
        function alignPictures(divName) {
            ...
            $(opts.selector + " " + divName).each(function(index, element) {
                if (element.clientHeight > divHeight) {
	    ...
	    // Fix anomaly when first loading div in jQueryUI tabs
	    // Note this is a kludge.  Will it bite me?
	    if (divHeight == 0) divHeight = 205;

            $(opts.selector + " " + divName).css('height', (divHeight+2)+'px');
        ...
        function photo(j, hidden, username) {
            ...
            {
                $html = $("<div class='pwi_photo' style='cursor: text;'/>");
                if ($youtubeId == "") {
                    $html.append("<a href='" + $thumbnail1.url + "' rel='lb-" +
...
console.log('Stored ' + j.georss$where.gml$Point.gml$pos.$t);
                    }
			...
			// BLC A little mod here to show filename on page
			var linkName = j.media$group.media$title.$t;
// console.log('Generated: ' + linkName);
			var linkURL = j.media$group.media$content[0].url;
                        $c += '<p><a href="' + j.media$group.media$content[0].url + '">' + linkName + '</a>';
                        }
                        $html.append($c);

			// Here is where we could extract the Point data
			// Also note j.media$group.media$content[0].url 
			//   contains the file name as the last element
			//
			// If this image is geotagged, extract the Point
			//   data and append to photo
			if (j.georss$where && (j.georss$where.gml$Point)) {
   			  var latLng = j.georss$where.gml$Point.gml$pos.$t.split(" ");
			  var latitude = parseFloat(latLng[0]);
			  var longitude = parseFloat(latLng[1]);
// console.log('Appending: ' + j.georss$where.gml$Point.gml$pos.$t);
			  // Stick it on the div
   			  $html.append(longitude + ', ' + latitude );
   			  } 
                    }
                }

		// Prepare to export image data
		// Set up array for geopoint
		var coordinates = [];
                coordinates.push(longitude);
                coordinates.push(latitude);
		// Add location array to map, then other data
		returnDataMap['coordinates'] = coordinates;
		returnDataMap['caption'] = $caption;
		returnDataMap['filename'] = linkName;
		returnDataMap['imageurl'] = linkURL;
		returnDataMap['date'] = $d;
// console.log('WTF: ' + JSON.stringify(returnDataMap));
        ...
        function albums(j) {
            ...
            // fewer albums-per-page then max so paging
            if ($albumCounter > settings.albumsPerPage) {
                ...
                $navRow.append($ppage);
		// Put together divs for individual page links
    ...
    $.fn.pwi.returnDataMap = {
	}

    // Here, hopefully, will be where we harvest EXIF/IPTC data later
    return { returnDataMap: $.fn.pwi.returnDataMap };
})(jQuery);
